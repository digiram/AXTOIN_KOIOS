/**
 * Tenant administrator: org-level subscription (plans with billing_scope **tenant**), v1 monthly catalog only.
 */
...
import {
  ...
  findTenantSubscriptionForSettingsView,
...
import { requireTenantAdmin } from "../plugins/tenant-admin.js";
...
export const registerTenantSubscriptionRoutes = async (app: FastifyInstance) => {
  app.get(
    "/subscription/catalog",
    { preHandler: requireTenantAdmin },
    async (_request, _reply) => {
      const plans = await listV1CatalogPlansForBillingScope("tenant");
  ...
  app.get(
    ...
    async (request, _reply) => {
      ...
      const [sub, settings] = await Promise.all([
        findTenantSubscriptionForSettingsView(tenantId),
        getPlatformSubscriptionSettingsRow()
      ]);
      return {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const existing = await findActiveLikeTenantSubscription(tenantId);
      if (existing) {
      ...
      if (!plan || plan.billingScope !== "tenant" || !isV1SubscriberPlan(plan.durationUnit, plan.durationCount)) {
        return reply.code(400).send({
          error: "invalid_plan",
          message: "Choose a tenant-scoped monthly catalog plan."
        });
      }
      if (plan.disabled) {
        return reply.code(400).send({
      ...
      try {
        ...
        request.log.warn({ err: e }, "stripe customer sync failed after tenant subscribe");
      }
      const fresh = (await findActiveLikeTenantSubscription(tenantId)) ?? next;
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!active) {
        return reply.code(404).send({ error: "not_found", message: "No active organization subscription." });
      }
