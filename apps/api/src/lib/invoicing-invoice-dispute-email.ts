/**
 * Server-rendered HTML email notifying the customer that an invoice dispute was recorded (MJML wrapper).
 */
...
import {
  ...
  renderInvoicingEmailCustomerNotePanel,
  renderInvoicingEmailIssuerFooter
} from "./invoicing-email-layout.js";

export type InvoicingInvoiceDisputeEmailRenderInput = {
...
const renderDisputeHeader = (input: {
  ...
  const metaCell = `<td valign="top" align="right" style="padding:${INVOICING_EMAIL_CONTENT_PAD_Y} ${INVOICING_EMAIL_CONTENT_PAD_X} ${INVOICING_EMAIL_CONTENT_PAD_Y} ${hasCompanyBlock ? "16px" : INVOICING_EMAIL_CONTENT_PAD_X};width:${hasCompanyBlock ? (hasLogo ? "42%" : "50%") : "100%"};">
      ${invoicingEmailRightText("Invoice disputed", "font-size:18px;font-weight:700;line-height:1.2;color:#b45309;text-transform:uppercase;letter-spacing:0.04em;")}
...
const buildDisputeMessage = (input: InvoicingInvoiceDisputeEmailRenderInput): string =>
  `We have recorded a dispute for invoice ${input.displayNumber}. We will review your dispute and follow up with you.`;

const renderInvoiceSummarySection = (input: {
  theme: ReturnType<typeof invoicingDocumentEmailThemeHex>;
  renderInput: InvoicingInvoiceDisputeEmailRenderInput;
}): string => {
...
export const buildInvoicingInvoiceDisputeEmailSubject = (input: {
  displayNumber: string;
  companyName: string | null;
}): string => {
  const company = input.companyName?.trim();
  return company
    ? `Invoice ${input.displayNumber} disputed — ${company}`
    : `Invoice ${input.displayNumber} disputed`;
};

export const renderInvoicingInvoiceDisputeEmailHtml = async (
  input: InvoicingInvoiceDisputeEmailRenderInput
): Promise<string> => {
  ...
  const message = buildDisputeMessage(input);
  ...
  const documentCard = `<div style="width:100%;max-width:${INVOICING_EMAIL_DOC_MAX_WIDTH};margin:0 auto;background-color:#ffffff;font-family:${INVOICING_EMAIL_FONT_STACK};color:#0f172a;">
    ...
    ${renderInvoicingEmailCustomerNotePanel({
      note: input.disputedInformation,
  ...
  const mjml = `<mjml>
  <mj-head>
    <mj-title>${escapeInvoicingEmailHtml("Invoice disputed")} — ${escapeInvoicingEmailHtml(input.displayNumber)}</mj-title>
    <mj-attributes>
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Invoice dispute email MJML compile failed: ${message}`);
  }
  return html;
};
