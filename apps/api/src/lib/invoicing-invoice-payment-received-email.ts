/**
 * Server-rendered HTML email confirming full invoice payment (MJML wrapper).
 */
...
export type InvoicingInvoicePaymentReceivedEmailRenderInput = {
...
const renderPaymentHeader = (input: {
  ...
  const metaCell = `<td valign="top" align="right" style="padding:${INVOICING_EMAIL_CONTENT_PAD_Y} ${INVOICING_EMAIL_CONTENT_PAD_X} ${INVOICING_EMAIL_CONTENT_PAD_Y} ${hasCompanyBlock ? "16px" : INVOICING_EMAIL_CONTENT_PAD_X};width:${hasCompanyBlock ? (hasLogo ? "42%" : "50%") : "100%"};">
      ${invoicingEmailRightText("Payment received", "font-size:18px;font-weight:700;line-height:1.2;color:#047857;text-transform:uppercase;letter-spacing:0.04em;")}
...
const renderPaymentSummary = (input: InvoicingInvoicePaymentReceivedEmailRenderInput): string => {
  ...
  const rows = [
    `<tr style="background:${summaryBg};">
      ...
      <td style="${valueStyle}font-weight:600;">${escapeInvoicingEmailHtml(input.displayNumber)}</td>
    </tr>`,
    `<tr style="background:${summaryBg};">
      <td style="${labelStyle}">Payment date</td>
      <td style="${valueStyle}">${escapeInvoicingEmailHtml(formatInvoicingIsoDate(input.paymentDate))}</td>
    </tr>`
  ];
  ...
  if (input.reference?.trim()) {
    rows.push(
      `<tr style="background:${summaryBg};">
        ...
        <td style="${valueStyle}">${escapeInvoicingEmailHtml(input.reference.trim())}</td>
      </tr>`
    );
  }
    ...
    `<tr>
      <td colspan="2" style="padding-top:12px;background:#ffffff;">
        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
          <tr>
            <td style="background:${theme.primary};color:#ffffff;font-size:14px;font-weight:600;padding:12px 16px;text-align:left;">Amount paid</td>
            <td style="background:${theme.totalHighlight};color:#ffffff;font-size:18px;font-weight:700;padding:12px 8px;text-align:right;white-space:nowrap;">${escapeInvoicingEmailHtml(formatInvoicingMoneyMinor(input.amountPaidMinor, input.currencyCode))}</td>
          </tr>
        </table>
      </td>
    </tr>`
  ...
  return invoicingEmailPaddedSection(
    `${invoicingEmailLeftText("Payment summary", "font-size:14px;font-weight:700;color:#0f172a;margin-bottom:10px;")}
     <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border:1px solid #e7e5e4;border-radius:12px;overflow:hidden;">
       ${rows.join("")}
     </table>`,
...
export const buildInvoicingInvoicePaymentReceivedEmailSubject = (input: {
  displayNumber: string;
  companyName: string | null;
}): string => {
  const company = input.companyName?.trim();
  return company
    ? `Payment received for invoice ${input.displayNumber} — ${company}`
    : `Payment received for invoice ${input.displayNumber}`;
};

export const renderInvoicingInvoicePaymentReceivedEmailHtml = async (
  input: InvoicingInvoicePaymentReceivedEmailRenderInput
): Promise<string> => {
  const theme = invoicingDocumentEmailThemeHex(input.documentThemeColor);
  ...
  const message = `Thank you. We have received your payment in full for invoice ${input.displayNumber}. This email confirms that your invoice is now paid.`;
  ...
  const documentCard = `<div style="width:100%;max-width:${INVOICING_EMAIL_DOC_MAX_WIDTH};margin:0 auto;background-color:#ffffff;font-family:${INVOICING_EMAIL_FONT_STACK};color:#0f172a;">
    ...
    ${invoicingEmailPaddedSection(
      `${invoicingEmailLeftText("Confirmation", "font-size:14px;font-weight:700;color:#0f172a;margin-bottom:8px;")}
       <div style="font-size:13px;line-height:1.5;color:#475569;text-align:left;">${escapeInvoicingEmailHtml(message)}</div>`,
  ...
  const mjml = `<mjml>
  <mj-head>
    <mj-title>${escapeInvoicingEmailHtml("Payment received")} — ${escapeInvoicingEmailHtml(input.displayNumber)}</mj-title>
    <mj-attributes>
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Invoice payment received email MJML compile failed: ${message}`);
  }
  return html;
};
