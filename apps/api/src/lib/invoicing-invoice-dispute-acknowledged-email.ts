/**
 * Server-rendered HTML email confirming the company acknowledged an invoice dispute (MJML wrapper).
 */
...
export type InvoicingInvoiceDisputeAcknowledgedEmailRenderInput = {
...
const renderAcknowledgmentHeader = (input: {
  ...
  const metaCell = `<td valign="top" align="right" style="padding:${INVOICING_EMAIL_CONTENT_PAD_Y} ${INVOICING_EMAIL_CONTENT_PAD_X} ${INVOICING_EMAIL_CONTENT_PAD_Y} ${hasCompanyBlock ? "16px" : INVOICING_EMAIL_CONTENT_PAD_X};width:${hasCompanyBlock ? (hasLogo ? "42%" : "50%") : "100%"};">
      ${invoicingEmailRightText("Dispute acknowledged", "font-size:18px;font-weight:700;line-height:1.2;color:#047857;text-transform:uppercase;letter-spacing:0.04em;")}
...
const buildAcknowledgmentMessage = (input: InvoicingInvoiceDisputeAcknowledgedEmailRenderInput): string =>
  `Your dispute for invoice ${input.displayNumber} has been acknowledged.`;

const renderInvoiceSummarySection = (input: {
  theme: ReturnType<typeof invoicingDocumentEmailThemeHex>;
  renderInput: InvoicingInvoiceDisputeAcknowledgedEmailRenderInput;
}): string => {
  ...
  if (renderInput.dueDate?.trim()) {
    rows.push(
      `<tr style="background:${summaryBg};">
        <td style="${labelStyle}">Due date</td>
        <td style="${valueStyle}">${escapeInvoicingEmailHtml(formatInvoicingIsoDate(renderInput.dueDate))}</td>
      </tr>`
    );
  }
...
export const buildInvoicingInvoiceDisputeAcknowledgedEmailSubject = (input: {
  displayNumber: string;
  companyName: string | null;
}): string => {
  const company = input.companyName?.trim();
  return company
    ? `Dispute acknowledged for invoice ${input.displayNumber} — ${company}`
    : `Dispute acknowledged for invoice ${input.displayNumber}`;
};

export const renderInvoicingInvoiceDisputeAcknowledgedEmailHtml = async (
  input: InvoicingInvoiceDisputeAcknowledgedEmailRenderInput
): Promise<string> => {
  ...
  const message = buildAcknowledgmentMessage(input);
  ...
  const documentCard = `<div style="width:100%;max-width:${INVOICING_EMAIL_DOC_MAX_WIDTH};margin:0 auto;background-color:#ffffff;font-family:${INVOICING_EMAIL_FONT_STACK};color:#0f172a;">
    ...
    ${renderInvoicingEmailNoteCard({
      ...
      text: input.companyResponse
    })}
    ${renderInvoicingEmailNoteCard({
      label: "Outstanding payment",
      text: input.outstandingPaymentPlan,
  ...
  const mjml = `<mjml>
  <mj-head>
    <mj-title>${escapeInvoicingEmailHtml("Dispute acknowledged")} — ${escapeInvoicingEmailHtml(input.displayNumber)}</mj-title>
    <mj-attributes>
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Invoice dispute acknowledgment email MJML compile failed: ${message}`);
  }
  return html;
};
