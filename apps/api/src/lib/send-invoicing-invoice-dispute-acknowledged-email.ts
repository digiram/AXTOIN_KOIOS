/**
 * Invoice dispute acknowledgment email.
 *
 * Sends the customer-facing email when a tenant acknowledges an invoice dispute
 * with company response and payment plan details.
 */
...
import {
  buildInvoicingInvoiceDisputeAcknowledgedEmailSubject,
  renderInvoicingInvoiceDisputeAcknowledgedEmailHtml
} from "./invoicing-invoice-dispute-acknowledged-email.js";
...
export type SendInvoicingInvoiceDisputeAcknowledgedEmailResult =
  | { ok: true }
  | { ok: false; status: 404 | 409 | 502 | 503; error: string; message: string };

export const sendInvoicingInvoiceDisputeAcknowledgedEmail = async (params: {
  tenantId: string;
  invoiceId: string;
  actorUserId: string | null;
  companyResponse: string;
  outstandingPaymentPlan: string;
  to?: string | null;
  log: FastifyBaseLogger;
}): Promise<SendInvoicingInvoiceDisputeAcknowledgedEmailResult> => {
  const { tenantId, invoiceId, actorUserId, companyResponse, outstandingPaymentPlan, log } = params;
  ...
  if (!invoicingEmailMomentIsEnabled(cfg, "dispute_acknowledged")) {
    return {
      ok: false,
      status: 409,
  ...
  const subject = buildInvoicingInvoiceDisputeAcknowledgedEmailSubject({
    displayNumber,
  ...
  let html: string;
  try {
    html = await renderInvoicingInvoiceDisputeAcknowledgedEmailHtml({
      displayNumber,
    ...
    log.error({ err, tenantId, invoiceId }, "invoice dispute acknowledgment email HTML render failed");
    return {
      ok: false,
      status: 502,
      error: "render_error",
      message: "Could not build the dispute acknowledgment email."
    };
  }
  ...
  try {
    ...
    log.warn({ err, tenantId, invoiceId, source }, "invoice dispute acknowledgment email SMTP send failed");
    return {
      ok: false,
      status: 502,
      error: "mail_error",
      message: "Could not send the dispute acknowledgment email. Check SMTP settings and try again."
    };
  }

  await insertInvoicingAuditEvent({
    tenantId,
    eventKind: "invoice_dispute_acknowledgment_email_sent",
    ...
    payload: {
      ...
      outstandingPaymentPlan: outstandingPaymentPlan.trim()
    }
  });
  ...
  if (actorUserId) {
    try {
      await deliverInternalMailboxMessage({
        ...
        source: "invoicing",
        subject: `Invoice ${displayNumber} dispute acknowledgment sent`,
        bodyHtml: `<p>You sent a dispute acknowledgment for invoice <strong>${displayNumber}</strong> to <strong>${to}</strong>.</p>`,
      ...
      log.warn(
        ...
        "internal mailbox notification skipped after invoice dispute acknowledgment email send"
      );
    }
  }
