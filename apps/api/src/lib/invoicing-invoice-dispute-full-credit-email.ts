/**
 * Server-rendered HTML email confirming a dispute was fully credited (MJML wrapper).
 */
...
export type InvoicingInvoiceDisputeFullCreditEmailRenderInput = {
...
const renderCreditHeader = (input: {
  ...
  const metaCell = `<td valign="top" align="right" style="padding:${INVOICING_EMAIL_CONTENT_PAD_Y} ${INVOICING_EMAIL_CONTENT_PAD_X} ${INVOICING_EMAIL_CONTENT_PAD_Y} ${hasCompanyBlock ? "16px" : INVOICING_EMAIL_CONTENT_PAD_X};width:${hasCompanyBlock ? (hasLogo ? "42%" : "50%") : "100%"};">
      ${invoicingEmailRightText("Credit applied", "font-size:18px;font-weight:700;line-height:1.2;color:#047857;text-transform:uppercase;letter-spacing:0.04em;")}
...
const buildCreditMessage = (input: InvoicingInvoiceDisputeFullCreditEmailRenderInput): string => {
  const credited = formatInvoicingMoneyMinor(input.creditedAmountMinor, input.currencyCode);
  return [
    `Your dispute for invoice ${input.displayNumber} has been fully credited for ${credited}.`,
    `You will receive a separate email with credited invoice ${input.revisedDisplayNumber}, showing a zero balance.`
  ].join("\n\n");
};

const renderCreditSummarySection = (input: {
  theme: ReturnType<typeof invoicingDocumentEmailThemeHex>;
  renderInput: InvoicingInvoiceDisputeFullCreditEmailRenderInput;
}): string => {
  ...
  const originalAmountMinor = renderInput.creditedAmountMinor;
  const creditAppliedMinor = renderInput.creditedAmountMinor;
  const remainingMinor = 0;
  ...
  const rows = [
    ...
    `<tr style="background:${summaryBg};">
      <td style="${labelStyle}">Credit date</td>
      <td style="${valueStyle}">${escapeInvoicingEmailHtml(formatInvoicingIsoDate(renderInput.creditDate))}</td>
    </tr>`,
    `<tr style="background:${summaryBg};">
      <td style="${labelStyle}">Original amount</td>
      <td style="${valueStyle}">${escapeInvoicingEmailHtml(formatInvoicingMoneyMinor(originalAmountMinor, renderInput.currencyCode))}</td>
    </tr>`,
    `<tr style="background:${summaryBg};">
      <td style="${labelStyle}">Credit applied</td>
      <td style="${valueStyle}">${escapeInvoicingEmailHtml(formatInvoicingMoneyMinor(creditAppliedMinor, renderInput.currencyCode))}</td>
    </tr>`,
    `<tr>
      <td colspan="2" style="padding-top:12px;background:#ffffff;">
        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
          <tr>
            <td style="background:${theme.primary};color:#ffffff;font-size:14px;font-weight:600;padding:12px 16px;text-align:left;">Total remaining</td>
            <td style="background:${theme.totalHighlight};color:#ffffff;font-size:18px;font-weight:700;padding:12px 8px;text-align:right;white-space:nowrap;">${escapeInvoicingEmailHtml(formatInvoicingMoneyMinor(remainingMinor, renderInput.currencyCode))}</td>
          </tr>
        </table>
      </td>
    </tr>`
  ...
  return invoicingEmailPaddedSection(
    `${invoicingEmailLeftText("Credit summary", "font-size:14px;font-weight:700;color:#0f172a;margin-bottom:10px;")}
     <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border:1px solid #e7e5e4;border-radius:12px;overflow:hidden;">
       ${rows.join("")}
     </table>`,
...
export const buildInvoicingInvoiceDisputeFullCreditEmailSubject = (input: {
  displayNumber: string;
  companyName: string | null;
}): string => {
  const company = input.companyName?.trim();
  return company
    ? `Full credit applied for invoice ${input.displayNumber} — ${company}`
    : `Full credit applied for invoice ${input.displayNumber}`;
};

export const renderInvoicingInvoiceDisputeFullCreditEmailHtml = async (
  input: InvoicingInvoiceDisputeFullCreditEmailRenderInput
): Promise<string> => {
  ...
  const message = buildCreditMessage(input);
  ...
  const documentCard = `<div style="width:100%;max-width:${INVOICING_EMAIL_DOC_MAX_WIDTH};margin:0 auto;background-color:#ffffff;font-family:${INVOICING_EMAIL_FONT_STACK};color:#0f172a;">
    ...
    ${note ? renderInvoicingEmailNoteCard({ label: "Note", text: note, extraSectionStyle: "padding-bottom:8px;border-bottom:1px solid #e7e5e4;" }) : ""}
    ${renderCreditSummarySection({ theme, renderInput: input })}
    ${renderInvoicingEmailIssuerFooter(input.issuerSnapshot, input.footerText)}
  </div>`;

  const mjml = `<mjml>
  <mj-head>
    <mj-title>${escapeInvoicingEmailHtml("Credit applied")} — ${escapeInvoicingEmailHtml(input.displayNumber)}</mj-title>
    <mj-attributes>
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Invoice dispute full credit email MJML compile failed: ${message}`);
  }
  return html;
};
