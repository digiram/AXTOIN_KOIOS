/**
 * User Subscription Settings panel.
 *
...
export const UserSubscriptionSettingsPanel = ({ authHeaders, refreshSession, logout }: Props) => {
  const { formatDateTimeUtc } = useUserDisplayDatetime();
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
  const [plans, setPlans] = useState<CatalogPlan[]>([]);
  const [sub, setSub] = useState<SubscriptionDto | null>(null);
  /** When false, platform does not require or accept new subscription sign-ups; full app use is allowed without a plan. */
  ...
  const subscribe = async (planId: string) => {
    ...
    setErr("");
    try {
      const res = await doFetch(`${API_BASE_URL}/account/subscription`, {
        method: "POST",
  ...
  const schedulePlanChange = async () => {
    ...
    setErr("");
    try {
      const res = await doFetch(`${API_BASE_URL}/account/subscription/schedule-plan-change`, {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  const clearScheduledPlanChange = async () => {
    ...
    setErr("");
    try {
      const res = await doFetch(`${API_BASE_URL}/account/subscription/scheduled-plan-change`, {
        method: "DELETE"
      });
      if (!res || !res.ok) {
        const j = (await res?.json().catch(() => null)) as { message?: string } | null;
  ...
  const cancel = async (effective: "immediate" | "period_end") => {
    ...
    setErr("");
    try {
      const res = await doFetch(`${API_BASE_URL}/account/subscription/cancel`, {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  if (loading) {
    return <p className="text-sm text-stone-500">Loading subscription…</p>;
  }

  return (
    <div className="space-y-6">
      <div className={settingsHeadingAccentClass}>
        <h2 className="text-base font-semibold text-slate-900">Your subscription</h2>
        <p className="mt-1 text-sm text-stone-600">
          Per-user catalog plans (monthly, UTC billing periods). Refunds for early cancellation are processed through
          your payment provider; this screen updates your in-app subscription state only.
        </p>
      </div>
      ...
      {sub ? (
        <div className="rounded-lg border border-stone-200 bg-stone-50/50 p-4 text-sm text-stone-800">
          ...
          <p className="mt-2 text-stone-700">
            Current period (UTC): {formatDateTimeUtc(sub.currentPeriodStart)} → {formatDateTimeUtc(sub.currentPeriodEnd)}
          </p>
          {activePlan ? (
          ...
          {sub.stripe?.card ? (
            ...
            <p className="mt-2 text-xs text-stone-500">
              Stripe publishable key is not configured. Ask a platform admin to enable Payments.
            </p>
          )}
          {sub.stripe?.setupIntentAvailable ? (
            <StripeSubscriptionCardForm
              setupIntentPath="/account/subscription/stripe/setup-intent"
          ...
          {canScheduleTierChange && (sub.status === "active" || sub.status === "canceling") ? (
            <div className="mt-4 space-y-2 rounded-lg border border-stone-200 bg-white p-3">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <label htmlFor="user-sub-next-plan" className="sr-only">
                  Target plan
                </label>
                <select
                  id="user-sub-next-plan"
        ...
        <p className="rounded-lg border border-stone-200 bg-stone-50/80 px-4 py-3 text-sm text-stone-700">
          Subscription billing is turned off for this platform. You can use the app without a paid plan. If your
          operator enables subscription billing later, catalog tiers will appear here.
        </p>
      ) : plans.length === 0 ? (
        <p className="text-sm text-stone-600">No per-user monthly plans are available in the catalog yet.</p>
      ) : (
